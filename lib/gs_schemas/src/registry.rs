...
mod test {
    ...
    pub fn serialize_registry() {
        ...
        let o_a = original.push_object(DummyObject(RegistryName::gs_const("a"))).unwrap();
        let o_b = original.push_object(DummyObject(RegistryName::gs_const("b"))).unwrap();
        let o_c = original.push_object(DummyObject(RegistryName::gs_const("c"))).unwrap();

        let mut original_rev: Registry<DummyObject> = Registry::default();

        let _or_c = original_rev
            .push_object(DummyObject(RegistryName::gs_const("c")))
            .unwrap();
        let _or_b = original_rev
            .push_object(DummyObject(RegistryName::gs_const("b")))
            .unwrap();
        let _or_a = original_rev
            .push_object(DummyObject(RegistryName::gs_const("a")))
            .unwrap();

        let mut missing: Registry<DummyObject> = Registry::default();

        let _m_a = missing.push_object(DummyObject(RegistryName::gs_const("a"))).unwrap();
        let _m_b = missing.push_object(DummyObject(RegistryName::gs_const("b"))).unwrap();
        ...
        match missing_result {
            Err(RegistryDeserializationError::MissingEntries(me)) => {
                assert_eq!(me, vec![RegistryName::gs_const("c")]);
            }
            Err(e) => {
