...
use crate::prelude::*;
use gs_schemas::player::{PlayerCharacter, PlayerAccount, AccountId, CharacterId};
...
impl Component for PlayerCharacterComponent {
    ...
    fn on_add() -> Option<ComponentHook> {
        Some(|mut world: DeferredWorld, ctx: HookContext| {
            let Some(avatar) = world.entity(ctx.entity).get::<PlayerCharacterComponent>() else {return};
            let character = avatar.character;
            let Some(mut cache) = world.get_resource_mut::<PlayerCache>() else {return};
        ...
        Some(|mut world: DeferredWorld, ctx: HookContext| {
            ...
            match cached {
                ...
                hashbrown::hash_map::Entry::Occupied(_) => {
                    error!("Could not remove player character {character:?} from the cache as the mapping points to a different character");
                }
